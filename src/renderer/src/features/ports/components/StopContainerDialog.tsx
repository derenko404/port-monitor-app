...
export function StopContainerDialog({
  ...
  onConfirm,
  onCancel
}: StopContainerDialogProps): React.JSX.Element {
      ...
      <AlertDialogContent size="sm" className="gap-0 p-5">
        ...
        <AlertDialogHeader className="items-center gap-3 text-center">
          ...
          <div className="flex flex-col items-center gap-1.5">
            <AlertDialogTitle className="text-sm">{t('dialogs.stop.title')}</AlertDialogTitle>
          ...
          <AlertDialogDescription
            className={cn('text-xs leading-relaxed', error && 'text-destructive')}
          >
            {error ? t(`dialogs.stop.${error}`) : t('dialogs.stop.desc')}
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter className="mt-5 flex-row gap-2">
          ...
          <AlertDialogAction
            ...
            disabled={busy}
            onClick={(e) => {
              e.preventDefault() // keep open through the stop + timeout
              onConfirm()
            }}
          >
            {busy ? (
              <>
                <Loader2 className="size-4 animate-spin" /> {t('common.stopping')}
              </>
            ) : (
              t('dialogs.stop.confirm')
            )}
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
...
export default StopContainerDialog
